...
from features_mpn import bond_features, atom_features
...
class SmilesDataset(dgl.data.DGLDataset):
    ...
    def process(self):
        ...
        for i, (raw_reactant_graph, raw_product_graph) in enumerate(self.raw_graphs):
            ...
            reactant_graph = mol_to_dgl(raw_reactant_graph)
            product_graph = mol_to_dgl(raw_product_graph)
...
def read_data(dataset, mode):
    ...
    with open(path) as f:
        for line in f.readlines():
            ...
            reactant_graph = Chem.MolFromSmiles(reactant_smiles)
            product_graph = Chem.MolFromSmiles(product_smiles)
...
def preprocess(dataset):
    ...
    train_graphs = read_data(dataset, 'train')
    valid_graphs = read_data(dataset, 'valid')
    test_graphs = read_data(dataset, 'test')

    return None, train_graphs, valid_graphs, test_graphs
...
def load_data(args):
    ...
    return 133, 147, train_dataset, valid_dataset, test_dataset
