...
def train(args, data):
    node_feature_len, bond_feature_len, train_data, valid_data, test_data = data
    # feature_len = sum([len(feature_encoder[key]) for key in data_processing.attribute_names])
    model = GNN(args.gnn, args.layer, node_feature_len, args.dim)
    ...
    for i in range(args.epoch):
        ...
        print('time used: {} : {}'.format(time.time()-t, time.strftime("%Y-%m-%d-%H_%M_%S", time.localtime())))
        t = time.time()
